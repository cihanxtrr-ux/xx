...
export default function Documents() {
  ...
  const handleDownloadTrigger = (item: DocumentItem) => {
  ...
  const filteredItems = documentItems.filter(item => {
    const title = t.documents.items[item.titleKey] || '';
    const desc = t.documents.items[item.descKey] || '';
    const matchesSearch = 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-6 border-b border-gray-150 dark:border-white/10">
        ...
        <div className="flex flex-wrap gap-2">
          {(['all', 'specs', 'bim', 'certificates'] as const).map((tab) => (
            ...
            >
              {tab === 'all' && t.documents.categories.all}
              {tab === 'specs' && t.documents.categories.specs}
              {tab === 'bim' && t.documents.categories.bim}
              {tab === 'certificates' && t.documents.categories.certificates}
            </button>
          ))}
        </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-base font-bold text-gray-900 dark:text-white leading-snug group-hover:text-[#0012FF] dark:group-hover:text-cyan-400 transition-colors">
                      {t.documents.items[item.titleKey]}
                    </h3>
                    <p className="text-xs text-gray-400 leading-relaxed min-h-[48px]">
                      {t.documents.items[item.descKey]}
                    </p>
                  </div>
                  ...
                  >
                    {downloadStatus === 'loading' && (
                      <>
                        ...
                        <span>{t.documents.downloadingBtn}</span>
                      </>
                    )}
                    {downloadStatus === 'completed' && (
                      <>
                        <CheckCircle2 className="h-3.5 w-3.5 text-emerald-500" />
                        <span>{t.documents.downloadedBtn}</span>
                      </>
                    )}
                    {downloadStatus === 'idle' && (
                      <>
