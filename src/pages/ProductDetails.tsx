  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTelemetryValues(prev => {
        ...
        } else if (product.id === 'prod-mcc') {
          return {
            motorLoad: (84.2 + delta * 0.5).toFixed(1) + ' %',
            contactorTemp: (45.1 + delta * 0.3).toFixed(1) + ' °C',
            overloadTripStatus: Math.random() > 0.995 ? 'TRIPPED' : 'NOMINAL',
            currentDraw: (412 + delta * 4).toFixed(1) + ' A',
            harmonicsTHD: (1.2 + delta * 0.05).toFixed(2) + ' %'
          };
        } else if (product.id === 'prod-ups') {
          return {
            inverterVoltage: (479.8 + delta * 0.4).toFixed(1) + ' V',
            loadSymmetry: (98.9 + delta * 0.1).toFixed(2) + ' %',
            batteryVoltage: (542.4 + delta * 0.8).toFixed(1) + ' V',
            thermalCoreTemp: (39.5 + delta * 0.15).toFixed(1) + ' °C',
            bypassActive: 'NO (DOUBLE CONVERSION)'
          };
        } else if (product.id === 'prod-pqa') {
          return {
            voltageSagIntensity: (0.12 + Math.random() * 0.02).toFixed(3) + ' %',
            activeHarmonicLimit: (4.1 + delta * 0.08).toFixed(2) + ' % (IEEE519)',
            measuredPowerFact: (0.985 + delta * 0.001).toFixed(3),
            instantaneousFlicker: (0.045 + delta * 0.002).toFixed(4),
            continuousRecTime: '4,120 Hours'
          };
        } else {
          return {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-8">
          ...
          <div className="relative overflow-hidden rounded-3xl aspect-[16/10] bg-gray-50 dark:bg-slate-950 border border-gray-150/50 dark:border-white/5 shadow-inner">
            {product && product.imageSrc ? (
              <motion.img 
                ...
                src={product.images && product.images[activeImageIndex] ? product.images[activeImageIndex] : product.imageSrc} 
                alt={product.name}
          ...
          {product && product.images && product.images.length > 1 && (
            <div className="flex gap-3 justify-start items-center overflow-x-auto pb-1 select-none">
              {product.images.map((imgUrl, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`relative w-20 sm:w-24 aspect-[16/10] rounded-2xl overflow-hidden bg-gray-50 dark:bg-slate-950 border transition-all cursor-pointer p-0 shrink-0 ${
                      isActive 
                        ? 'border-[#0012FF] dark:border-cyan-400 ring-2 ring-[#0012FF]/10 dark:ring-cyan-400/20' 
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-mono text-gray-400 uppercase tracking-widest block font-bold">{t.contact.phoneNumber}</label>
                    <input
                      required
                      type="tel"
                      placeholder="e.g. +353 1 902 4499"
                      value={clientPhone}
                  ...
                  <div className="pt-2">
                    <MathCaptchaCheckbox onVerified={setVerified} className="max-w-full mb-3" />
                  </div>
                  ...
                  <div className="pt-2">
                    <button
                      type="submit"
                      disabled={!verified}
                      className="w-full py-3 px-4 rounded-xl bg-gray-900 dark:bg-[#00FF00] dark:text-gray-950 text-white font-bold hover:bg-gray-800 dark:hover:bg-[#00E000] text-xs tracking-wider transition-all uppercase cursor-pointer border-0 active:scale-[0.98] shadow-sm flex items-center justify-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
