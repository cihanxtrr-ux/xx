          ...
          {inquiries.length === 0 ? (
            ...
            inquiries.map((inq) => (
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-[11px] text-gray-500 pt-2 border-t border-gray-100 gap-2 font-sans w-full">
                  <div className="flex flex-col gap-0.5 text-left">
                    <span className="flex items-center gap-1 text-gray-700 leading-tight">
                      <User className="h-3.5 w-3.5 text-gray-400 flex-shrink-0" />
                      <strong>{inq.clientName}</strong> ({inq.clientEmail})
                    </span>
                    {inq.clientPhone && (
                      <span className="text-[10px] text-gray-500 font-mono pl-4.5 flex items-center gap-1 leading-none">
                        <span>📞</span> {inq.clientPhone}
                      </span>
                    )}
                  </div>
                  <span className="flex items-center gap-1 text-[10px] font-mono text-gray-400 shrink-0 self-end sm:self-center">
