...
export default function BasketDrawer({ onAddInquiry }: BasketDrawerProps) {
  ...
  const handleSubmitRfq = (e: React.FormEvent) => {
    e.preventDefault();
    if (basket.length === 0 || !name || !email || !phone) return;
    if (!verified) return;
              ...
              <div className="space-y-6">
                ...
                <form onSubmit={handleSubmitRfq} className="border-t border-gray-150 dark:border-white/10 pt-6 space-y-4">
                  ...
                  <button
                    type="submit"
                    disabled={!verified}
                    className="w-full py-3 px-4 rounded-xl bg-[#0012FF] dark:bg-cyan-400 hover:bg-opacity-90 text-white dark:text-slate-950 font-bold text-xs transition uppercase flex items-center justify-center gap-2 cursor-pointer border-0 mt-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
