import { TranslationDictionary } from '../translations';

export interface ProductItem {
  id: string;
  name: string;
  desc: string;
  leadTime: number;
  specs: string[];
  estimatedCost: number;
  voltage: string;
  amperage: number;
  discipline: 'industrial' | 'renewable' | 'datacenter' | 'commercial';
  imageSrc: string;
  images?: string[];
  subCategory: string;
}

export function getProductCollection(t: TranslationDictionary): ProductItem[] {
  return [
    {
      id: 'prod-mvs',
      name: t.products.p1Name,
      desc: t.products.p1Desc,
      leadTime: 8,
      specs: [
        'Voltage Rating: up to 24kV Max continuous feed',
        'Current Rating: 1250A - 2500A Busbar rating',
        'Breaker: Highly reliable vacuum circuit breakers',
        'Enclosure: IEC 62271-200 Arc-resistant metal-clad config',
      ],
      estimatedCost: 85000,
      voltage: '13.8kV',
      amperage: 1200,
      discipline: 'industrial',
      subCategory: 'distribution',
      imageSrc: '/src/assets/images/mvs_switchgear_1779335480278.png',
      images: [
        '/src/assets/images/mvs_switchgear_1779335480278.png',
        '/src/assets/images/hero_electrical_1779333030707.png',
        '/src/assets/images/project_data_center_1779333053234.png'
      ],
    },
    {
      id: 'prod-mcc',
      name: t.products.p6Name,
      desc: t.products.p6Desc,
      leadTime: 7,
      specs: [
        'Voltage Rating: 480V structural sizing',
        'Standard: IEC 61439-2 Form 4 Type 7 segregated protection',
        'Relays: Smart electronic overload monitor nodes',
        'PLC Interface: Modbus, Profinet gateway integrations',
      ],
      estimatedCost: 112000,
      voltage: '480V',
      amperage: 1600,
      discipline: 'industrial',
      subCategory: 'motor-control',
      imageSrc: '/src/assets/images/mvs_switchgear_1779335480278.png',
      images: [
        '/src/assets/images/mvs_switchgear_1779335480278.png',
        '/src/assets/images/transfer_switch_1779335521007.png',
        '/src/assets/images/project_green_energy_1779333073620.png'
      ],
    },
    {
      id: 'prod-megapack',
      name: t.products.p2Name,
      desc: t.products.p2Desc,
      leadTime: 16,
      specs: [
        'Total Cap.: 5.0 Megawatt-hours modular liquid-cooled',
        'Inverter Ratio: Bi-directional high frequency coupling',
        'C-Rate: 1C rapid dispatch peak-shaving system',
        'Safety: Double structural fire barrier enclosure & thermal runaways',
      ],
      estimatedCost: 1150000,
      voltage: '277/480V',
      amperage: 2000,
      discipline: 'renewable',
      subCategory: 'storage',
      imageSrc: '/src/assets/images/grid_batteries_1779335502733.png',
      images: [
        '/src/assets/images/grid_batteries_1779335502733.png',
        '/src/assets/images/project_green_energy_1779333073620.png',
        '/src/assets/images/hero_electrical_1779333030707.png'
      ],
    },
    {
      id: 'prod-charger',
      name: t.products.p4Name,
      desc: t.products.p4Desc,
      leadTime: 10,
      specs: [
        'Charging rate: Liquid cooled up to 350kW rapid DC output',
        'Connector ties: Dual CCS / NACS connections dynamic scheduling',
        'Power alignment: Integrated grid power-factor corrections',
        'Enclosure: Zero-vibration IP66 cast alloy shell',
      ],
      estimatedCost: 65000,
      voltage: '277/480V',
      amperage: 400,
      discipline: 'renewable',
      subCategory: 'ev-infrastructure',
      imageSrc: '/src/assets/images/dc_charger_1779335540876.png',
      images: [
        '/src/assets/images/dc_charger_1779335540876.png',
        '/src/assets/images/project_green_energy_1779333073620.png',
        '/src/assets/images/hero_electrical_1779333030707.png'
      ],
    },
    {
      id: 'prod-ats',
      name: t.products.p3Name,
      desc: t.products.p3Desc,
      leadTime: 6,
      specs: [
        'Amp limits: 400A to 4000A structural scale options',
        'Transition delay: Built under 4ms latency switching limits',
        'Isolation level: Form 4 Type 7 segregated busbars',
        'Control board: Integrated double fiber telemetry gateway',
      ],
      estimatedCost: 48000,
      voltage: '277/480V',
      amperage: 800,
      discipline: 'datacenter',
      subCategory: 'switching',
      imageSrc: '/src/assets/images/transfer_switch_1779335521007.png',
      images: [
        '/src/assets/images/transfer_switch_1779335521007.png',
        '/src/assets/images/project_data_center_1779333053234.png',
        '/src/assets/images/hero_electrical_1779333030707.png'
      ],
    },
    {
      id: 'prod-ups',
      name: t.products.p7Name,
      desc: t.products.p7Desc,
      leadTime: 9,
      specs: [
        'Architecture: True online 3-phase double conversion topology',
        'Efficiency: Up to 99% in premium ECO mode configuration',
        'Maintenance: 100% hot-swappable modular components',
        'Telemetry: Dual gigabit SCADA connections & active control loop',
      ],
      estimatedCost: 210000,
      voltage: '480V',
      amperage: 1200,
      discipline: 'datacenter',
      subCategory: 'ups-systems',
      imageSrc: '/src/assets/images/transfer_switch_1779335521007.png',
      images: [
        '/src/assets/images/transfer_switch_1779335521007.png',
        '/src/assets/images/bms_controller_1779335563008.png',
        '/src/assets/images/project_data_center_1779333053234.png'
      ],
    },
    {
      id: 'prod-bms',
      name: t.products.p5Name,
      desc: t.products.p5Desc,
      leadTime: 5,
      specs: [
        'Interfaces: Gigabit Ethernet fiber ports + RS485 loops',
        'Monitoring: Dual core digital system boards thermal breakers',
        'Standards: BACnet, Modbus TCP, OPC UA native protocols',
        'Screen panel: 10 inch high contrast industrial diagnostic display',
      ],
      estimatedCost: 19500,
      voltage: '120/208V',
      amperage: 100,
      discipline: 'commercial',
      subCategory: 'automation',
      imageSrc: '/src/assets/images/bms_controller_1779335563008.png',
      images: [
        '/src/assets/images/bms_controller_1779335563008.png',
        '/src/assets/images/terminal_connector_1779902850105.png',
        '/src/assets/images/transfer_switch_1779335521007.png'
      ],
    },
    {
      id: 'prod-pqa',
      name: t.products.p8Name,
      desc: t.products.p8Desc,
      leadTime: 4,
      specs: [
        'Accuracy: Class 0.2S precision power quality parameters recording',
        'Harmonics: Up to 63rd order voltage and current analysis',
        'Thermal Probe: Enclosure temperature and humidity alarms',
        'Integration: Direct REST/JSON telemetry streams & Modbus Master TCP',
      ],
      estimatedCost: 34000,
      voltage: '120/208V',
      amperage: 400,
      discipline: 'commercial',
      subCategory: 'monitoring',
      imageSrc: '/src/assets/images/bms_controller_1779335563008.png',
      images: [
        '/src/assets/images/bms_controller_1779335563008.png',
        '/src/assets/images/transfer_switch_1779335521007.png',
        '/src/assets/images/terminal_connector_1779902850105.png'
      ],
    },
    {
      id: 'prod-terminal-block',
      name: t.products.p9Name,
      desc: t.products.p9Desc,
      leadTime: 2,
      specs: [
        'Type: Reusable 8-Port compact push-in conductor splicing block',
        'Capacity: Up to 8 independent wires with integrated cage-clamp springs',
        'Ratings: Up to 450V / 32A structural rating with 4kV surge boundaries',
        'Conductors: Compliant with solid/stranded/flexible core wiring 0.2 to 4.0 mm²',
      ],
      estimatedCost: 125,
      voltage: '450V',
      amperage: 32,
      discipline: 'commercial',
      subCategory: 'automation',
      imageSrc: '/src/assets/images/terminal_connector_1779902850105.png',
      images: [
        '/src/assets/images/terminal_connector_1779902850105.png',
        '/src/assets/images/bms_controller_1779335563008.png',
        '/src/assets/images/transfer_switch_1779335521007.png'
      ],
    },
  ];
}
